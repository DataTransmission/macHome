  ...
  implicit none
  real*8,intent(out):: var !output an integer like nx
  integer            :: varID, i, status
  integer,intent(in) :: ncID
  character(len=*),intent(in):: varname
    status= nf90_inq_varID(ncID, varname, varID)
  ...
  implicit none
  real*8,allocatable,intent(out),dimension(:):: var !output an integer like nx
  integer            :: varID, i, dimIDs(1), status
  integer,intent(in) :: ncID
  integer:: vecLen
  character(len=*),intent(in):: varname
...
!  real*8,allocatable,dimension(:,:,:):: tempvar
  real*8,allocatable,intent(out),dimension(:,:):: var !output an integer like nx
  integer            :: varID, i,  status
  integer,intent(in) :: ncID
  character(len=*),intent(in):: varname
...
!    var = tempvar(1,:,:,:)
  ...
  subroutine writenc_real3d(varname,var,ncID,nx,ny,nz)
  implicit none
  integer,intent(in):: nx, ny, nz
  real*8,intent(in):: var(nx,ny,nz) !output an integer like nx
  integer            :: varID, i, status
  integer,intent(in) :: ncID
  !integer:: vecLen(3)
  character(len=*),intent(in):: varname
      !status = nf90_open(filename,nf90_Write, ncID)
      status = nf90_inq_varID(ncID, varname , varID)           !status = nf90_get_var(ncID, varID, 'U')
      status = nf90_put_var(ncID, varID, var)
  return
  end subroutine writenc_real3d
