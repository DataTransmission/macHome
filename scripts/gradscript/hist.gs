***************************************************************************************
*	$Id: hist.gs,v 1.4 2010/03/28 04:55:00 bguan Exp bguan $
...
*
lines=8
line.1='DSET ^hist.dat~'
line.2='UNDEF 'default_undef()
line.3=_xdef
line.4=_ydef
line.5=zdef
line.6='TDEF 1 LINEAR '_tims' '_dtim
line.7='VARS 1'
line.8='ENDVARS'
cnt=1
while(cnt<=lines-1)
status=write(mytmpdir'/hist.ctl~',line.cnt)
cnt=cnt+1
endwhile
cnt=1
while(cnt<=1)
varline=output' 'nz' 99 Add description here.'
status=write(mytmpdir'/hist.ctl~',varline)
cnt=cnt+1
endwhile
status=write(mytmpdir'/hist.ctl~',line.lines)
status=close(mytmpdir'/hist.ctl~')
