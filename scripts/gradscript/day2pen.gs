...
'set gxout fwrite'
'set fwrite 'mytmpdir'/day2pen.dat~'
...
'set time 01JAN'_yrs' 31DEC'_yre
qdims()
pentads=(_yre-_yrs+1)*73
writectl(mytmpdir'/day2pen.ctl~','^day2pen.dat~',pentads,num_var,name)

'set time 01JAN'_yrs

pentad=1
while(pentad<=pentads)
offset1=(pentad-1)*5
offset2=offset1+4
vcnt=1
while(vcnt<=num_var)
...
while(zcnt<=_ze)
'set z 'zcnt
'pentmp=ave(dy2pntmp'vcnt',t+'offset1',t+'offset2')'
...
if(_.file.1!='')
writectl(_.path.1'/'_.file.1'.ctl','^'_.file.1'.dat',pentads,num_var,name)
...
line.7='TDEF 'nt' LINEAR '_tims' 5dy'
line.8='VARS 'nv
line.9='ENDVARS'
cnt=1
while(cnt<=lines-1)
