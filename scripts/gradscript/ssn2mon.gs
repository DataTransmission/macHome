***************************************************************************************
*	$Id: ssn2mon.gs,v 1.27 2008/07/02 22:24:04 bguan Exp bguan $
*	Copyright (C) 2006 Bin Guan.
*	Distributed under GNU/GPL.
***************************************************************************************
function ssn2mon(arg)
*
...
months=seasons*3
writectl(mytmpdir'/monlz.ctl~','^monlz.dat~',months,num_var,name)
...
offset=season-1
mrepeat=3
if(season=1); mrepeat=2; endif
if(season=seasons+1); mrepeat=1; endif
mcnt=1
while(mcnt<=mrepeat)
vcnt=1
while(vcnt<=num_var)
...
while(zcnt<=_ze)
'set z 'zcnt
'montmp=sn2mntmp'vcnt'(t+'offset')'
'display const(montmp,'_.undef.1',-u)'
zcnt=zcnt+1
endwhile
vcnt=vcnt+1
endwhile
...
if(_.file.1!='')
writectl(_.path.1'/'_.file.1'.ctl','^'_.file.1'.dat',months,num_var,name)
'!cp 'mytmpdir'/monlz.dat~ '_.path.1'/'_.file.1'.dat'
endif
...
line.6='TDEF 'nt' LINEAR '_tims' 1mo'
line.7='VARS 'nv
line.8='ENDVARS'
cnt=1
while(cnt<=lines-1)
...
*
say '  Seasonal-to-monthly regridding.'
say ''
say '  Usage: ssn2mon -v <var1> [<var2>...] [-n <name1> [<name2>...]] [-u <undef>] [-o <file>] [-p <path>]'
say '     <var>: seasonal variable.'
say '     <name>: name for monthly variable. Same as <var> if unset.'
...
say '        Output data will start from Jan and ends at Dec (the last Dec always has missing values).'
say ''
