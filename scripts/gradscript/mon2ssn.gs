***************************************************************************************
*	$Id: mon2ssn.gs,v 1.27 2008/07/02 01:58:39 bguan Exp bguan $
*	Copyright (C) 2004 Bin Guan.
*	Distributed under GNU/GPL.
***************************************************************************************
function mon2ssn(arg)
*
...
writectl(mytmpdir'/ssnlz.ctl~','^ssnlz.dat~',seasons,num_var,name)

'set time JAN'_yrs

season=1
while(season<=seasons)
offset1=(season-1)*3-1
offset2=offset1+2
vcnt=1
while(vcnt<=num_var)
zcnt=_zs
while(zcnt<=_ze)
'set z 'zcnt
'ssntmp=ave(mn2sntmp'vcnt',t+'offset1',t+'offset2')'
'display const(ssntmp,'_.undef.1',-u)'
zcnt=zcnt+1
endwhile
vcnt=vcnt+1
endwhile
...
if(_.file.1!='')
writectl(_.path.1'/'_.file.1'.ctl','^'_.file.1'.dat',seasons,num_var,name)
'!cp 'mytmpdir'/ssnlz.dat~ '_.path.1'/'_.file.1'.dat'
endif
...
line.6='TDEF 'nt' LINEAR '_tims' 3mo'
line.7='VARS 'nv
line.8='ENDVARS'
cnt=1
while(cnt<=lines-1)
...
*
say '  Monthly-to-seasonal regridding.'
say ''
say '  Usage: mon2ssn -v <var1> [<var2>...] [-n <name1> [<name2>...]] [-u <undef>] [-o <file>] [-p <path>]'
say '     <var>: monthly variable.'
say '     <name>: name for seasonal variable. Same as <var> if unset.'
